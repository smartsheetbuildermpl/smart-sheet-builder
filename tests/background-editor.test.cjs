...
(async()=>{
  ...
  try {
    ...
    await open();
    const editorBox=await editor.boundingBox();assert.deepEqual([editorBox.x,editorBox.y,editorBox.width,editorBox.height],[0,0,1440,1000],'editor uses the full browser viewport');
    assert(await page.evaluate(()=>document.body.classList.contains('bg-editor-open')),'page scroll is locked while the editor is open');
    assert.equal(await editor.locator('details.bg-advanced').evaluate(e=>e.open),false,'advanced cleanup starts collapsed');
    assert(await editor.locator('[data-tool-panel="wand"]').isVisible());assert.equal(await editor.locator('[data-tool-panel="erase"]').isVisible(),false);
    await act('Erase Brush').click();assert(await editor.locator('[data-tool-panel="erase"]').isVisible());assert.equal(await editor.locator('[data-tool-panel="wand"]').isVisible(),false);
    await act('Restore Brush').click();assert(await editor.locator('[data-tool-panel="restore"]').isVisible());
    await act('Pan').click();assert(await editor.locator('[data-tool-panel="pan"]').isVisible());
    await act('Magic Wand').click();
    ...
    await open();
    assert.equal(await editor.locator('[data-action="auto-edge"]').count(),1);
    ...
    // The compact X confirms before discarding a real, unapplied edit. Cancel keeps the editor open.
    await open();await act('Erase Brush').click();await slider('#bg-size',6);await clickPixel(60,40);
    page.once('dialog',dialog=>dialog.dismiss());await editor.getByRole('button',{name:'Close Background Editor'}).click();assert.equal(await editor.count(),1);
    page.once('dialog',dialog=>dialog.accept());await editor.getByRole('button',{name:'Close Background Editor'}).click();assert.equal(await editor.count(),0);
    // Opening and closing the true full-screen mode returns to the builder's previous page position.
    await page.evaluate(()=>window.scrollTo(0,420));const scrollBefore=await page.evaluate(()=>window.scrollY);
    await page.evaluate(()=>document.querySelector('.background-edit-btn').click());await editor.waitFor();
    assert.equal(await page.evaluate(()=>document.elementFromPoint(2,2).closest('.bg-editor')!==null),true,'editor covers the page at the viewport edge');
    await act('Cancel').click();await page.waitForFunction(()=>!document.querySelector('.bg-editor'));
    assert.equal(await page.evaluate(()=>window.scrollY),scrollBefore,'closing restores builder scroll position');
