...
(async () => {
  ...
  try {
    ...
    const passwords = fs.readFileSync('supabase-password-recovery-v1.sql','utf8');
    await db.exec(passwords); await db.exec(passwords);
    const beforePassword = await one('select to_jsonb(p) p from profiles p where id=$1',[newcomer]);
    const ticket = (await one('select ssb_create_password_recovery($1) ticket',[newcomer])).ticket;
    assert.equal((await one('select ssb_claim_password_recovery($1,$2) claimed',[basic,ticket])).claimed,false,'ticket bound to recovery identity');
    assert.equal((await one('select ssb_claim_password_recovery($1,$2) claimed',[newcomer,ticket])).claimed,true);
    assert.equal((await one('select ssb_claim_password_recovery($1,$2) claimed',[newcomer,ticket])).claimed,false,'single use even if cookie replayed');
    const expired = (await one('select ssb_create_password_recovery($1) ticket',[newcomer])).ticket;
    await q("update password_recovery_tickets set expires_at=now()-interval '1 second' where id=$1",[expired]);
    assert.equal((await one('select ssb_claim_password_recovery($1,$2) claimed',[newcomer,expired])).claimed,false);
    await q('select ssb_record_password_change($1)',[newcomer]);
    const audit = await one("select * from user_access_history where user_id=$1 and event='password_changed_by_user'",[newcomer]);
    assert.equal(audit.actor_id,newcomer);assert.equal(audit.old_value,null);assert.equal(audit.new_value,null);assert(audit.created_at);
    assert.deepEqual(await one('select to_jsonb(p) p from profiles p where id=$1',[newcomer]),beforePassword,'password flow never changes profile, role, entitlement or usage');
    await db.exec('set role authenticated');
    await denied('select * from password_recovery_tickets');
    await denied('select ssb_create_password_recovery($1)',[newcomer]);
    await denied('select ssb_record_password_change($1)',[newcomer]);
    await db.exec('reset role');
